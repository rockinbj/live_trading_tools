...
def closePositionForce(exchange, markets, openPositions, symbol=None, close_pct=1.0):
    ...
    for s, pos in openPositions.iterrows():
        ...
        para = {
            ...
            "type": "MARKET",
            "quantity": exchange.amount_to_precision(s, pos["contracts"] * close_pct),
            "reduceOnly": True,
        }
...
def draw_indexcta_pic(index_file="", index_name="", len_short="", len_long="", bias=False):
    if not index_file:
        return
    ...
    plt.annotate(f"BIAS({last_bias:.3f})", xy=(_dt.iloc[-1], index_df.iloc[-1]['ma_short']),
                 xytext=(-60, 0), textcoords='offset points', fontsize=12, color='white')
